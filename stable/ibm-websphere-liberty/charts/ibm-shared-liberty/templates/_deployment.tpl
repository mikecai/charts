...
{{- define "slt.deployment" -}}
  {{- $params := . -}}
  {{- $root := first $params -}}
  {{ include "slt.utils.isMicroProfileHealthEnabled" (list $root) }}
---
...
{{- end }}
  annotations:
  {{ if $root.Values.app.autoCreate }}
{{ include "slt.utils.app-nav-annotations" (list $root) | indent 4 }}
  {{ end }}
...
{{ toYaml $root.Values.image.readinessProbe | indent 10 }}
        {{- else }}
          httpGet:
          {{- if $root.Values.isMicroProfileHealthEnabled }}
            path: /health
          {{- else }}
            path: /
          {{- end }}
