...
{{/*
Creates a boolean value named "isMicroProfileHealthEnabled" that determines if MicroProfile Health is supported AND is enabled.
*/}}
{{- define "slt.utils.isMicroProfileHealthEnabled" -}}
  {{- $params := . -}}
  {{- $root := first $params -}}
{{- if $root.Values.microprofile -}}
{{- if $root.Values.microprofile.health -}}
{{- if $root.Values.microprofile.health.enabled -}}
{{- $_ := set $root.Values "isMicroProfileHealthEnabled" true -}}
{{- end -}}
{{- end -}}
{{- end -}}
{{- end -}}
...
Create a service name, defaulting to fullname if not provided.
*/}}
{{- define "slt.utils.app-nav-annotations" -}}
  {{- $params := . -}}
  {{- $root := first $params -}}
  {{- $stateful := or $root.Values.logs.persistTransactionLogs $root.Values.logs.persistLogs -}}
prism.subkind: Liberty
prism.app.auto-create.version: "{{ $root.Values.app.version }}"
prism.app.auto-create.labels-values: {{ include "slt.utils.fullname" (list $root) }}
{{ end -}}